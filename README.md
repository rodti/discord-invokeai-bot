...
Each result includes Refresh, Edit prompt, Random, Tweak, and Delete controls. The Tweak panel loads model and LoRA choices from the connected InvokeAI installation and scheduler choices from InvokeAI's API schema; dropdown pages expose installations with more than 25 options. Refreshing or changing a result creates a new message and preserves the original image; only Delete removes it. Only the requester may use the controls. A yellow face alternates between closed and wide-open mouth frames as it munches through the “Working…” progress bar; very occasionally a ghost gives chase a few cells behind. Result metadata uses icons for model, sampler, seed, dimensions, steps, and CFG/guidance. Discord receives only a generic failure message; full errors are logged to the bot console.
